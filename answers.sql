...
SELECT brands.name, brands.founded FROM brands LEFT JOIN models ON (brands.name = models.brand_name) WHERE brands.name NOT IN (SELECT brand_name FROM models);
...
SELECT brands.name, models.name, models.year, brands.discontinued, (brands.discontinued - models.year) AS years_until_brand_discontinued FROM models LEFT JOIN brands ON (models.brand_name = brands.name) WHERE brands.discontinued IS NOT NULL ORDER BY brands.name, models.name, models.year;
